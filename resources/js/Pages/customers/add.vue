<template>
    <div class="flex flex-col gap-4">
        <div class="flex justify-between items-center">
            <h1 class="font-semibold text-3xl">Add Customer</h1>
        </div>
        <form @submit.prevent="forms.post(route('customers.store'))" class="bg-white p-4 rounded-sm flex flex-col gap-4" method="POST">
            <div>
                <p>Code</p>
            ...
            <div>
                <p>Phone</p>
                <input :class="{'border border-red-500': forms.errors.phone}" type="text" v-model="forms.phone" :disabled="forms.processing">
                <p v-if="forms.errors.phone" class="text-red-500">
                    {{ forms.errors.phone }}
                </p>
            </div>
            <div class="flex justify-between">
                <Link :href="route('customers.index')" class="border border-primary py-2 text-sm px-4" :class="{'pointer-events-none': forms.processing}">
                    Back
                </Link>
